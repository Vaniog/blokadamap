from sqlalchemy import Date, ForeignKey, SmallInteger, Text, UniqueConstraint, Table, Column
...
class Diary(Base):
    ...
    author_id = mapped_column(SmallInteger, ForeignKey('author.author_id'), unique=True, nullable=False)
...
NoteToTag = Table('note_to_tag', Base.metadata,
    Column('note_id', SmallInteger, ForeignKey('note.note_id'), primary_key=True, nullable=False),
    Column('tag_id', SmallInteger, ForeignKey('tag.tag_id'), primary_key=True, nullable=False)
)

NoteToPoint = Table('note_to_point', Base.metadata,
    Column('note_id', SmallInteger, ForeignKey('note.note_id'), primary_key=True, nullable=False),
...
class Note(Base):
    ...
    note_type_id = mapped_column(SmallInteger, ForeignKey('note_type.note_type_id'), nullable=False)
    temporality_id = mapped_column(SmallInteger, ForeignKey('temporality.temporality_id'), nullable=False)
